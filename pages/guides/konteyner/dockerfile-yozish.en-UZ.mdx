...
`WORKDIR` konteyner ichidagi ishchi papkani belgilaydi. Bundan keyingi barcha `RUN`, `COPY`, `CMD` va boshqa instruksiyalar shu papkada bajariladi.
...
### COPY va ADD — Fayllarni ko'chirish
...
`ENV` konteyner ichidagi muhit o'zgaruvchilarini belgilaydi. Bu qiymatlar build jarayonida ham, konteyner ishlayotganda ham mavjud bo'ladi.
...
### ARG — Build-time o'zgaruvchilar

`ARG` faqat **build jarayonida** mavjud bo'lgan o'zgaruvchilarni belgilaydi. Konteyner ishlayotganda ular mavjud bo'lmaydi.
...
### CMD va ENTRYPOINT — Konteyner ishga tushishi
...
### VOLUME — Doimiy ma'lumotlar

`VOLUME` konteyner ichida doimiy (persistent) ma'lumotlarni saqlash uchun mount point yaratadi. Konteyner o'chirilsa ham, volume'dagi ma'lumotlar saqlanib qoladi.
...
### HEALTHCHECK — Konteyner sog'lig'ini tekshirish
...
| Foydalanuvchi | root (default) | `USER appuser` |
| Build | Single-stage | Multi-stage build |
| Sog'lik | HEALTHCHECK yo'q | `HEALTHCHECK --interval=30s ...` |
| Hajm | Keshni tozalamagan | `rm -rf /var/lib/apt/lists/*` |
| Maxfiylik | `.env` imageda | `.dockerignore` + runtime env |
| Instruksiyalar | Ko'p `RUN` qatorlar | `&&` bilan birlashtirilgan |
